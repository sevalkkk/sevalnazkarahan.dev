...
export async function POST(request: Request) {
  try {
    ...
    if (action === "send_otp") {
      ...
      // Yöntem A: Resend ile Ziyaretçiye Şık HTML Kod Gönder
      if (resendApiKey) {
        try {
            ...
            html: `
              <div style="font-family: Arial, sans-serif; background-color: #0d0d0d; color: #ffffff; padding: 32px; border-radius: 16px; max-width: 500px; margin: 0 auto; border: 1px solid #262626;">
                ...
                <p style="color: #cccccc; font-size: 14px; line-height: 1.6;">
                  ...
                  Seval Naz Karahan portfolyo sitesi üzerinden mesajınızı iletebilmek için e-posta sahipliği onay kodunuz:
                </p>
                ...
                <p style="color: #888888; font-size: 12px; margin: 0;">
                  Bu kod <strong>5 dakika</strong> boyunca geçerlidir. Eğer bu talebi siz yapmadıysanız lütfen bu e-postayı dikkate almayın.
                </p>
              </div>
            `,
          });
      ...
      // Yöntem B: Doğrudan Ziyaretçinin Kendi Mailine FormSubmit ile Kod Gönderimi
      if (!otpSent) {
        try {
          const formSubmitRes = await fetch(`https://formsubmit.co/ajax/${normalizedEmail}`, {
            method: "POST",
            headers: {
          ...
          if (formSubmitRes.ok) otpSent = true;
        } catch (err) {
        ...
        try {
          ...
          const emailHtml = `
            <div style="font-family: Arial, sans-serif; background-color: #0d0d0d; color: #ffffff; padding: 32px; border-radius: 16px; max-width: 600px; margin: 0 auto; border: 1px solid #262626;">
              ...
              <div style="margin-bottom: 20px;">
                <p style="color: #888888; font-size: 12px; text-transform: uppercase; margin: 0 0 4px 0;">Onaylı E-posta Adresi</p>
          ...
          if (!error) messageDelivered = true;
        } catch (err) {
      ...
      if (!messageDelivered) {
        try {
          const formSubmitRes = await fetch(`https://formsubmit.co/ajax/${receiverEmail}`, {
            ...
            body: JSON.stringify({
              _subject: `[Doğrulanmış Mesaj] ${trimmedName} (${normalizedEmail}) yeni bir mesaj gönderdi`,
              _replyto: normalizedEmail,
              _template: "table",
              _captcha: "false",
              "Güvenlik Durumu": "E-posta Kodu ile Doğrulandı (OTP Verified ✓)",
              "Gönderen": trimmedName,
              "E-posta": normalizedEmail,
              "Mesaj": trimmedMessage,
              "Tarih": new Date().toLocaleString("tr-TR", { timeZone: "Europe/Istanbul" }),
            }),
          });
          if (formSubmitRes.ok) messageDelivered = true;
        } catch (err) {
